              ...
              {AI_STACK.map((t, i) => (
                <div key={i} style={{ border: '1px solid hsl(var(--border))', borderRadius: 14, padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 11, background: t.color, overflow: 'hidden', position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    {t.logoFile && <img src={`/logos/${t.logoFile}`} alt={t.name} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'contain', padding: 5, display: 'none' }} onLoad={e => { const img = e.target as HTMLImageElement; img.style.display = 'block'; const span = img.previousElementSibling as HTMLElement; if (span) span.style.display = 'none'; const parent = img.parentElement as HTMLElement; if (parent) parent.style.background = '#fff'; }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />}
                  </div>
