            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {AI_STACK.map((t, i) => (
                <div key={i} style={{ border: '1px solid hsl(var(--border))', borderRadius: 14, padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 11, background: t.logoFile ? '#fff' : t.color, overflow: 'hidden', position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {t.logoFile ? (
                      <>
                        <img src={`/logos/${t.logoFile}`} alt={t.name} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'contain', padding: 5 }} onError={e => { const img = e.target as HTMLImageElement; img.style.display = 'none'; const sib = img.nextElementSibling as HTMLElement; if (sib) sib.style.display = 'block'; const p = img.parentElement as HTMLElement; if (p) p.style.background = t.color; }} />
                        <span style={{ position: 'absolute', fontSize: 11, fontWeight: 700, letterSpacing: '-0.02em', color: t.textColor, display: 'none' }}>{t.initials}</span>
                      </>
                    ) : (
